function getTexts() {
    var y = document.getElementById("pssource").value;
    var urlVariable = encodeURI ("../getTextList.xq?source=" + y);
    
    var textQuery = new XMLHttpRequest();
    
    textQuery.open("GET", urlVariable, true);
    textQuery.send();
    
    textQuery.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            var metreQuery = new XMLHttpRequest();
            var urlMetreVariable = encodeURI("../getMetreList.xq?source=" + y);
            
            metreQuery.open("GET", urlMetreVariable, true);
            metreQuery.send();
            
            metreQuery.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    document.getElementById("psTextList").innerHTML = textQuery.responseText;
                    document.getElementById("psMetreList").innerHTML = metreQuery.responseText;
                }
            }
        }
    } 
...
function getTunes() {
    ...
    document.getElementById("selMet").value = metre;
        ...
        if (this.readyState == 4 && this.status == 200) {
          ...
          verseQuery.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
              ...
              document.getElementById("submit").innerHTML = "<button type='button' class='submitbtn' onclick='verovioApp()'><i class='fa fa-arrow-right fa-2x' aria-hidden='true'></i><br/>Go!</button>";
...
/*function verseGenerator(verses) {
    ...
    outputHTML = outputHTML + "<br/><i>Select a minimum of two</i>"
...
function setTexts(){
    var y = document.getElementById("selMet").value;
    var urlVariable = encodeURI("getTexts.xq?metre=" + y);
    ...
    textQuery.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            document.getElementById("psList").innerHTML = textQuery.responseText;
        }
    }
