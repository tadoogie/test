function setOptions() {
  ...
  if (pWidth < 1270){
      pageWidth = (pWidth-30) * 100 / zoom ;
  }else{
      pageWidth = 1270 * 100 / zoom;
  }
  
  options = {
              pageHeight: pageHeight,
              pageWidth: (pageWidth-100) *.96,
              scale: zoom,
...
function loadFile() {
  xmlData = renderPsalm()
  fullData = new XMLSerializer().serializeToString(xmlData)
  loadData(fullData)
};
    ...
    if (this.readyState == 4 && this.status == 200) {
      ...
      xhttp.onreadystatechange = function() {
        ...
        for (i = 0; i< sylCount; i++){
          ...
          //Ready next stanza to fill existing syllable containers 
          var thisContainer = project.getElementsByClassName(containerPos[i])
        ...
        k = k + 1;
            ...
            if (halfStanza === false){
                ...
                for (i = startCount; i< sylCount; i++){
                  ...
                  //Ready first stanza to fill existing syllable containers 
                  var thisContainer = project.getElementsByClassName(containerPos[i])
                  var newVerse, repeatVerse = xmlDoc.createElement("verse", project.namespaceURI);
                  var newSyl, repeatSyl = xmlDoc.createElement("syl", project.namespaceURI);
                  var newLyr = newLyric[k].childNodes[0].nodeValue;
                  var newLyrLen = newLyr.length;
                  var lastChar = newLyr.charAt(newLyrLen-1);
                  var newText = xmlDoc.createTextNode(newLyr);
