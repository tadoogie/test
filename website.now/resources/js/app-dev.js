...
const pdfDependencyLoadPromises = {};
...
function getPdfDocumentConstructor() {
    ...
    for (let i = 0; i < rootCandidates.length; i++) {
        const ctor = extractPdfConstructor(rootCandidates[i]);
...
function extractPdfConstructor(candidate) {
    ...
    if (typeof candidate.PDFDocument === 'function') {
        return candidate.PDFDocument;
    }
    if (typeof candidate.default === 'function') {
        return candidate.default;
    }
    if (candidate.default && typeof candidate.default.PDFDocument === 'function') {
        return candidate.default.PDFDocument;
    }
    return null;
}

function getBlobStreamFactory() {
    if (typeof window.blobStream === 'function') {
        return window.blobStream;
    }
...
function getSvgToPdfRenderer() {
    if (typeof window.SVGtoPDF === 'function') {
        return window.SVGtoPDF;
    }
...
function loadScriptOnce(source) {
    if (pdfDependencyLoadPromises[source]) {
        return pdfDependencyLoadPromises[source];
    }

    pdfDependencyLoadPromises[source] = new Promise(function(resolve, reject) {
        ...
        delete pdfDependencyLoadPromises[source];
        throw error;
    });
...
async function ensureScriptAvailable(sources, isReady) {
    if (isReady()) {
        return;
    }
    ...
    if (!isReady()) {
        throw lastError || new Error('Required script could not be initialized.');
    }
}

async function ensurePDFDependencies(requireSvgRenderer) {
    await ensureScriptAvailable(
        [
            'resources/js/pdfkit.standalone.js',
            'https://cdn.jsdelivr.net/npm/pdfkit@0.17.2/js/pdfkit.standalone.js',
            'https://unpkg.com/pdfkit@0.17.2/js/pdfkit.standalone.js'
        ],
        function() {
            return typeof getPdfDocumentConstructor() === 'function';
        }
    );

    await ensureScriptAvailable(
        [
            'resources/js/blob-stream.js',
            'https://cdn.jsdelivr.net/npm/blob-stream@0.1.3/blob-stream.js',
            'https://unpkg.com/blob-stream@0.1.3/blob-stream.js'
        ],
        function() {
            return typeof getBlobStreamFactory() === 'function';
        }
    );

    if (requireSvgRenderer) {
        await ensureScriptAvailable(
            [
                'resources/js/svg-to-pdfkit.js',
                'https://cdn.jsdelivr.net/npm/svg-to-pdfkit@0.1.8/source.js',
                'https://unpkg.com/svg-to-pdfkit@0.1.8/source.js'
            ],
            function() {
                return typeof getSvgToPdfRenderer() === 'function';
            }
        );
    }

    return {
        PDFDocument: getPdfDocumentConstructor(),
        blobStreamFactory: getBlobStreamFactory(),
        svgToPdfRenderer: requireSvgRenderer ? getSvgToPdfRenderer() : null
    };
}

function applyLineStrokeFallbacks(svgElement) {
    const lineLikeSelector = [
        '.stem',
        '.staffline',
        '.staffLine',
        '.ledgerline',
    ...
    const lineLikeElements = svgElement.querySelectorAll(lineLikeSelector);
    lineLikeElements.forEach(function(element) {
        const tag = (element.tagName || '').toLowerCase();
        const isBarline = element.classList && (element.classList.contains('barline') || element.classList.contains('barLine'));
        const isStem = element.classList && element.classList.contains('stem');
        const fillAttr = (element.getAttribute('fill') || '').toLowerCase();
        const fillStyle = (element.style.fill || '').toLowerCase();
        const hasSolidFill = (fillAttr && fillAttr !== 'none' && fillAttr !== 'transparent') ||
            (fillStyle && fillStyle !== 'none' && fillStyle !== 'transparent');
        const isPathWithNoFill = tag === 'path' && (
            fillAttr === 'none' ||
            fillStyle === 'none'
        );
        const isLinePrimitive = tag === 'line' || tag === 'polyline' || isPathWithNoFill;

        if (isBarline && hasSolidFill && !isLinePrimitive) {
            // Keep final/thick barlines as solid blocks.
        ...
        if (isStem && !isLinePrimitive) {
            // Oblique stems are often filled shapes; keep them solid.
            if (!element.getAttribute('fill') && !element.style.fill) {
                element.style.fill = 'currentColor';
            }
        ...
        const rawStrokeWidth = element.getAttribute('stroke-width') || element.style.strokeWidth || '';
        const strokeWidth = parseFloat(rawStrokeWidth);

        if (!element.getAttribute('stroke') && !element.style.stroke) {
            element.style.stroke = 'currentColor';
        }
        if (!Number.isFinite(strokeWidth) || strokeWidth < 0.6) {
...
function prepareSvgForPdf(svgString) {
    if (typeof svgString !== 'string' || svgString.indexOf('<svg') === -1 || !document.body) {
        return {
            svgInput: svgString,
            cleanup: function() {}
        };
    ...
    applyLineStrokeFallbacks(svgElement);
...
async function generateTextOnlyPDF(selectedPaperSize) {
    ...
    try {
        const deps = await ensurePDFDependencies(false);
...
async function generatePDF(selectedPaperSize) {
    ...
    try {
        const deps = await ensurePDFDependencies(true);
        const PDFDocument = deps.PDFDocument;
        const blobStreamFactory = deps.blobStreamFactory;
        const svgToPdfRenderer = deps.svgToPdfRenderer;
        ...
        for (let i = 1; i <= pageCount; i++) {
            ...
            const preparedSvg = prepareSvgForPdf(svgString);
            try {
                await svgToPdfRenderer(doc, preparedSvg.svgInput, doc.page.margins.left, doc.page.margins.top + currentPaperSettings.svgVerticalOffset, {
                    width: drawableWidth,
